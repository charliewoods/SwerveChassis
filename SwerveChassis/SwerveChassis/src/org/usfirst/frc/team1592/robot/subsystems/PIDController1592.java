 ...
 */
public class PIDController1592 implements PIDInterface, LiveWindowSendable
{
	public static final double kDefaultPeriod = .02;
	private static int instances = 0;
	private double m_P; // factor for "proportional" control
	private double m_I; // factor for "integral" control
	private double m_IZone = 1.0; // maximum control output from integrator
	 ...
	 *        default is 50ms.
	 */
	public PIDController1592(double Kp, double Ki, double Kd, double Kf, PIDSource source,
	 ...
	 *        default is 50ms.
	 */
	public PIDController1592(double Kp, double Ki, double Kd, PIDSource source, PIDOutput output,
			double period)
	{
	...
	/**
	 * Allocate a PID object with the given constants
	 * @param k class container holding constants
	 * @param source The PIDSource object that is used to get values
	 * @param output The PIDOutput object that is set to the output percentage
	 */
	public PIDController1592(PIDConstants k, PIDSource source, PIDOutput output) {
		this(k.kP,k.kI,k.kD,k.kF,source,output,k.period);
		this.setIZone(k.iZone);
	}
	...
	protected void calculate()
	{
		...
		if(enabled) {
			...
			synchronized(this){
				...
				//TODO: if we ever used this, integrator logic should be changed
				if(m_pidInput.getPIDSourceType().equals(PIDSourceType.kRate)) {
					...
					if(m_I != 0) {
						...
						double potentialIOutput = (m_totalError + m_error) * m_I;
						//If integral contribution will exceed iZone, do not accumulate more error
						if (Math.abs(potentialIOutput) < m_IZone) {
							double potentialOutput = potentialIOutput + m_P * m_error + m_D * (m_error - m_prevError);
							//Clamp the integrator if the output is saturated.  Otherwise, allow the integrated error to decay back to zero
							boolean outputNotSaturated = potentialOutput < m_maximumOutput && potentialOutput > m_minimumOutput; 
							if(outputNotSaturated) {
								m_totalError += m_error;
							} else if (m_error > 0 && m_totalError < 0){
								m_totalError = Math.min(m_totalError + m_error,0);
							} else if (m_error < 0 && m_totalError > 0) {
							 m_totalError = Math.max(m_totalError + m_error,0);
							}
						} 
					}
				}
	...
	/**
	 * Get the Integral action zone
	 *$
	 * @return integral action max
	 */
	...
	/**
	 * Set the integral zone
	 *$
	 * @param iZone output from integral contribution
	 */
	public synchronized void setIZone(double iZone)
	{
		// limit iZone from [0,1]
		if (iZone > 1) {
			iZone = 1;
		} else if (iZone < 0) {
			iZone = 0;
